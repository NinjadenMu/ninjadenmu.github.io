...
To optimize the map to favor one party, we could simulate an election with the new district set and see if the party we're favoring wins.  However, simulated annealing needs to be able to measure slow incremental progress - remember, we are only making tiny changes.  Checking the number of districts our favored party wins is the opposite of slow, constant progress - instead, progress is made in big jumps of winning an extra district.  Therefore, because this method can't diffrentiate between a slightly better and slightly worse districting, our simulated annealing algorithm will just randomly stumble around, unable to make incremental progress towards a good solution.  
